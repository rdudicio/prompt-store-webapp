...
def test_get_prompts_db(db_session_mock):
    mock_prompts = [
        models.Prompt(id=1, title="P1", text="T1", tags="t1", created_at=datetime.now(), updated_at=datetime.now()),
        models.Prompt(id=2, title="P2", text="T2", tags="t2", created_at=datetime.now(), updated_at=datetime.now()),
    ]
...
def test_get_prompt_by_id_db(db_session_mock):
    mock_prompt = models.Prompt(id=1, title="Single DB Prompt", text="Text", tags="single", created_at=datetime.now(), updated_at=datetime.now())
...
def test_update_prompt_db(db_session_mock):
    existing_prompt = models.Prompt(id=1, title="Old Title", text="Old Text", tags="old", created_at=datetime.now(), updated_at=datetime.now())
...
def test_delete_prompt_db(db_session_mock):
    existing_prompt = models.Prompt(id=1, title="Delete Me", text="Text", tags="delete", created_at=datetime.now(), updated_at=datetime.now())
    db_session_mock.query.return_value.filter.return_value.first.return_value = existing_prompt
